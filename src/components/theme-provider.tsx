...
const themeModeSchema = ['light', 'dark', 'auto']
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- used for type inference
const resolvedThemeSchema = ['light', 'dark']
const themeKey = 'theme'

type ThemeMode = (typeof themeModeSchema)[number]
type ResolvedTheme = (typeof resolvedThemeSchema)[number]
...
const getStoredThemeMode = createIsomorphicFn()
  ...
  .client((): ThemeMode => {
    try {
      ...
      return themeModeSchema.includes(storedTheme as ThemeMode)
...
const setStoredThemeMode = createClientOnlyFn((theme: ThemeMode) => {
  try {
    const parsedTheme = themeModeSchema.includes(theme) ? theme : 'auto'
...
export function ThemeProvider({ children }: ThemeProviderProps) {
  ...
  const toggleMode = (mode?: ThemeMode) => {
    setTheme(mode ? mode : getNextTheme(themeMode))
  }
